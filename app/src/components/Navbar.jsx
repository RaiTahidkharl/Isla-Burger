    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 flex items-center justify-between h-20">
        <a href="#home" className="flex items-center gap-3 group">
          <div className="relative w-12 h-12 rounded-2xl bg-gradient-to-br from-[#FFB627] to-[#E63946] flex items-center justify-center shadow-[0_8px_18px_-6px_rgba(230,57,70,0.5)] group-hover:rotate-12 transition-transform duration-500">
            <span className="text-2xl">🍔</span>
            <span className="absolute -top-1 -right-1 w-3 h-3 bg-white rounded-full"></span>
          </div>
          <div className="leading-tight min-w-0 max-w-[9rem] sm:max-w-none">
            <div className="font-display text-xl lg:text-2xl text-[#1A1206]">Isla Burger</div>
            <div className="text-[9px] lg:text-[10px] uppercase tracking-[0.15em] lg:tracking-[0.25em] text-[#D62828] font-bold truncate">
              {t.brandTagline}
            </div>
          </div>
        </a>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center bg-[#FFF8E7] rounded-full p-1 border-2 border-[#FCE4B6]">
            {langs.map(l => (
          ...
          <button onClick={onCartOpen} className="relative p-3 rounded-2xl bg-[#FFF8E7] border-2 border-[#FCE4B6] hover:border-[#FFB627] hover:-translate-y-0.5 transition-all">
            <ShoppingBag className="w-5 h-5 text-[#1A1206]" />
            {cartCount > 0 && (
          ...
          <a
            ...
            className="hidden md:inline-flex btn-3d text-xs xl:text-sm whitespace-nowrap shrink-0 items-center gap-1.5"
          >
      ...
      {menuOpen && (
        <div className="lg:hidden bg-[#FFF8E7] border-t-2 border-[#FCE4B6] px-5 py-4">
          ...
          <nav className="flex flex-col gap-2">
            ...
            <a
              ...
              className="flex items-center justify-between gap-2 px-4 py-3 rounded-xl bg-[#E63946] text-white font-bold uppercase text-sm tracking-wider"
            >
              <span>{t.nav.orderNow}</span>
